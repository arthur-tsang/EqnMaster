...
from rnn_encdec import RNNEncDec
...
if __name__ == '__main__':
    ...
    rnn = RNNEncDec(vdim, hdim, wdim, outdim, alpha=alpha, rho = rho)

    if sys.argv[10] == 'retrain':
        print 'Retraining'
        rnn.load_model(model_filename) # if retraining
    rnn.sgd(batch_size, n_epochs, X_train, Y_train, X_dev=X_dev, Y_dev=Y_dev, verbose=True, update_rule='momentum', filename=model_filename)
    rnn.save_model(model_filename)
